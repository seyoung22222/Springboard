...
public class BoardController {
	...
	public void setTemplate(JdbcTemplate template) {
		...
		/*
		template을 다른 클래스에서 사용하기 위해 static으로 선언한 변수에 
		할당한다. static(정적)변수는 프로그램 시작시 로딩되어 객체생성없이 
		클래스명 만으로 접근할 수 있는 특징이 있다.
		 */
		JdbcTemplateConst.template = this.template;
	}
	
	/*
	 멤버변수로 선언하여 클래스에서 전역저긍로 사용할 수 있다.
	...
	public String list(Model model, HttpServletRequest req) {
		...
		/*
		 사용자로부터 받은 모든 요청은 request개체에 저장되고, 이를
	...
	/*
	 글쓰기 처리에 대해 매핑처리를 한다. 전송방식은 post로 전송시 요청명은 value
	 속성에 기술한다.
	 */
	...
	public String writeAction(Model model, HttpServletRequest req,
		...
		/*
		 ...
		 받을수 있다. Spring에서 이와같은 객체를 '커맨드객체'라고 한다.
	...
	public String view(Model model, HttpServletRequest req) {
		...
		/*
		 목록에서 제목을 클릭할떄 전달되는 파라미터 idx, pageNum등을 
	...
	public String password(Model model, HttpServletRequest req) {
		/*
		 파라미터로 전달된 값 중에서 일련번호만 request내장객체를 통해 받은 후 
		 모델객체에 저장한다. 나머지 값은 View에서 직접 EL을 통해 받을 것이다.
		 */
	...
	public String editAction(HttpServletRequest req, Model model,
		...
		/*
		 수정처리가 완료되면 상세페이지로 이동하게 되는데 이떄 idx와 
