...
public class JDBCTemplateDAO {
	...
	/*
	 ...
	 JdbcTemplateConst.template에 할당하였으므로, DB작업을 
	 DAO에서도 수행할 수 있다.
	 */
	...
	public int write(SpringBoardDTO SpringBoardDTO) {
		...
		/*
		 insert와 같이 행의 변화가 생기는 쿼리문을 실행할때는 update()메서드를 
		 사용한다. PreparedStatementCreator()를 익명클래스로 정의하여 메서드를
		 오버라이딩 한 후 인파라미터가 있는 쿼리문에 값을 설정한 후 실행한다.
		 */
		int result = template.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) 
				...
				//메서드가 실행한 후 결과 값을 반환한다.
				return psmt;
			}
		});
		/*
		 insert쿼리를 실행한 결과는 1아니면 0이다. 따라서 해당 메서드의 
		 반환타입은 int로 설정되어 있다. 
		 */
		return result;
	}
	...
	public void updateHit(String idx) {
		...
		/*
		 ...
		 두번째 인수로 PreparedStatementSetter 익명클래스로
		 정의한다. 오버라이딩 된 메서드를 이용해서 인파라미터를 설정한다.
		 */
		...
		/*
		 update() 메서드는 int형의 반환갑싱 있지만 필요없는 경우에는
		 생략할 수 있다.
		 */
	}
